...
pub struct fluid_voice_t {
    ...
    pub check_sample_sanity_flag: i32,
    pub output_rate: f32,
    pub start_time: u32,
    pub ticks: u32,
    ...
    pub a1_incr: f32,
    pub a2_incr: f32,
    pub filter_coeff_incr_count: i32,
    pub pan: f32,
    pub amp_left: f32,
    pub amp_right: f32,
    pub reverb_send: f32,
...
pub unsafe fn new_fluid_voice(output_rate: f32) -> *mut fluid_voice_t {
    ...
    (*voice).volenv_data[FLUID_VOICE_ENVSUSTAIN as i32 as usize].count =
        0xffffffff as u32;
    ...
    (*voice).volenv_data[FLUID_VOICE_ENVFINISHED as i32 as usize].count =
        0xffffffff as u32;
    ...
    (*voice).modenv_data[FLUID_VOICE_ENVSUSTAIN as i32 as usize].count =
        0xffffffff as u32;
    ...
    (*voice).modenv_data[FLUID_VOICE_ENVFINISHED as i32 as usize].count =
        0xffffffff as u32;
...
pub unsafe fn delete_fluid_voice(voice: *mut fluid_voice_t) -> i32 {
    if voice.is_null() {
...
pub unsafe fn fluid_voice_gen_set(
...
pub unsafe fn fluid_voice_write(
    ...
    if !((*voice).volenv_section == FLUID_VOICE_ENVDELAY as i32) {
        ...
        match current_block {
            ...
            _ => {
                (*voice).amp_incr =
                    (target_amp - (*voice).amp) / 64 as i32 as libc::c_float;
                if !((*voice).amp == 0.0f32 && (*voice).amp_incr == 0.0f32) {
                    ...
                    if (*voice).phase_incr == 0 as i32 as libc::c_float {
                    ...
                    } else if fres < 5 as i32 as libc::c_float {
                    ...
                    if f64::abs((fres - (*voice).last_fres) as f64) > 0.01f64 {
                        ...
                        //       printf("Setting initial filter coefficients.\n");
                            ...
                            (*voice).a1_incr =
                                (a1_temp - (*voice).a1) / 64 as i32 as libc::c_float;
                            (*voice).a2_incr =
                                (a2_temp - (*voice).a2) / 64 as i32 as libc::c_float;
                            (*voice).b02_incr =
                                (b02_temp - (*voice).b02) / 64 as i32 as libc::c_float;
                            (*voice).b1_incr =
                                (b1_temp - (*voice).b1) / 64 as i32 as libc::c_float;
...
unsafe fn fluid_voice_effects(
    ...
    if !dsp_chorus_buf.is_null() && (*voice).amp_chorus != 0 as i32 as libc::c_float {
        dsp_i = 0 as i32;
        while dsp_i < count {
...
pub unsafe fn fluid_voice_get_channel(voice: *mut fluid_voice_t) -> *mut Channel {
    return (*voice).channel;
}

pub unsafe fn fluid_voice_start(mut voice: *mut fluid_voice_t) {
...
pub unsafe fn fluid_voice_calculate_runtime_synthesis_parameters(
    ...
    if !(*(*voice).channel).tuning.is_null() {
        ...
        (*voice).gen[GEN_PITCH as i32 as usize].val = (*tuning).pitch
            [60 as i32 as usize]
            + (*voice).gen[GEN_SCALETUNE as i32 as usize].val / 100.0f32 as f64
                * ((*tuning).pitch[(*voice).key as usize]
                    - (*tuning).pitch[60 as i32 as usize])
    } else {
        (*voice).gen[GEN_PITCH as i32 as usize].val =
            (*voice).gen[GEN_SCALETUNE as i32 as usize].val
                * ((*voice).key as i32 as libc::c_float - 60.0f32) as f64
...
pub unsafe fn calculate_hold_decay_buffers(
    ...
    if is_decay != 0 {
        ...
        if timecents > 5000 as i32 as libc::c_float {
    ...
    buffers = (((*voice).output_rate * seconds / 64 as i32 as f32) as f64 + 0.5f64)
        as i32;
    return buffers;
}

pub unsafe fn fluid_voice_update_param(mut voice: *mut fluid_voice_t, gen: i32) {
    let mut q_d_b;
    let mut x;
    let mut y;
    let mut count;
    ...
    match gen {
        ...
        16 => {
            (*voice).reverb_send = ((*voice).gen[GEN_REVERBSEND as i32 as usize].val
                as f32
        ...
        15 => {
            (*voice).chorus_send = ((*voice).gen[GEN_CHORUSSEND as i32 as usize].val
                as f32
        ...
        58 => {
            if (*voice).gen[GEN_OVERRIDEROOTKEY as i32 as usize].val
                > -(1 as i32) as f64
            {
                //FIXME: use flag instead of -1
                (*voice).root_pitch =
                    ((*voice).gen[GEN_OVERRIDEROOTKEY as i32 as usize].val
                        * 100.0f32 as f64
        ...
        5 => {
            (*voice).modlfo_to_pitch = (*voice).gen[GEN_MODLFOTOPITCH as i32 as usize].val
                as f32
        ...
        13 => {
            (*voice).modlfo_to_vol = (*voice).gen[GEN_MODLFOTOVOL as i32 as usize].val
                as f32
        ...
        10 => {
            (*voice).modlfo_to_fc = (*voice).gen[GEN_MODLFOTOFILTERFC as i32 as usize].val
                as f32
                + (*voice).gen[GEN_MODLFOTOFILTERFC as i32 as usize].mod_0 as f32
                + (*voice).gen[GEN_MODLFOTOFILTERFC as i32 as usize].nrpn as f32;
            (*voice).modlfo_to_fc =
                if (*voice).modlfo_to_fc < -(12000 as i32) as libc::c_float {
                    -(12000 as i32) as libc::c_float
                } else if (*voice).modlfo_to_fc > 12000 as i32 as libc::c_float {
                    12000 as i32 as libc::c_float
                } else {
                    (*voice).modlfo_to_fc
                };
        ...
        22 => {
            ...
            (*voice).modlfo_incr = 4.0f32 * 64 as i32 as libc::c_float * fluid_act2hz(x)
                / (*voice).output_rate;
            current_block_195 = 5267916556966421873;
        }
        24 => {
            ...
            (*voice).viblfo_incr = 4.0f32 * 64 as i32 as libc::c_float * fluid_act2hz(x)
                / (*voice).output_rate;
            current_block_195 = 5267916556966421873;
        }
        23 => {
        ...
        6 => {
            (*voice).viblfo_to_pitch = (*voice).gen[GEN_VIBLFOTOPITCH as i32 as usize].val
                as f32
        ...
        7 => {
            (*voice).modenv_to_pitch = (*voice).gen[GEN_MODENVTOPITCH as i32 as usize].val
                as f32
        ...
        11 => {
            (*voice).modenv_to_fc = (*voice).gen[GEN_MODENVTOFILTERFC as i32 as usize].val
                as f32
        ...
        0 | 4 => {
            if !(*voice).sample.is_null() {
                (*voice).start = (*(*voice).sample)
                    ...
                    .wrapping_add(
                        (32768 as i32
                            * ((*voice).gen[GEN_STARTADDRCOARSEOFS as i32 as usize].val
                                as f32
                                + (*voice).gen[GEN_STARTADDRCOARSEOFS as i32 as usize].mod_0
                                    as f32
                                + (*voice).gen[GEN_STARTADDRCOARSEOFS as i32 as usize].nrpn
        ...
        1 | 12 => {
            if !(*voice).sample.is_null() {
                (*voice).end = (*(*voice).sample)
                    ...
                    .wrapping_add(
                        (32768 as i32
                            * ((*voice).gen[GEN_ENDADDRCOARSEOFS as i32 as usize].val
                                as f32
                                + (*voice).gen[GEN_ENDADDRCOARSEOFS as i32 as usize].mod_0
                                    as f32
                                + (*voice).gen[GEN_ENDADDRCOARSEOFS as i32 as usize].nrpn
                                    as f32) as i32) as u32,
                    ) as i32;
                (*voice).check_sample_sanity_flag = (1 as i32) << 0 as i32
            }
                ...
                (*voice).loopstart = (*(*voice).sample)
                    ...
                    .wrapping_add(
                        ((*voice).gen[GEN_STARTLOOPADDROFS as i32 as usize].val as f32
                            + (*voice).gen[GEN_STARTLOOPADDROFS as i32 as usize].mod_0
                                as f32
                            + (*voice).gen[GEN_STARTLOOPADDROFS as i32 as usize].nrpn
                                as f32) as i32 as u32,
                    )
                    .wrapping_add(
                        (32768 as i32
                            * ((*voice).gen[GEN_STARTLOOPADDRCOARSEOFS as i32 as usize].val
                                as f32
                                + (*voice).gen[GEN_STARTLOOPADDRCOARSEOFS as i32 as usize]
                                    .mod_0 as f32
                                + (*voice).gen[GEN_STARTLOOPADDRCOARSEOFS as i32 as usize]
                                    .nrpn as f32) as i32)
                            as u32,
        ...
        3 | 50 => {
            if !(*voice).sample.is_null() {
                (*voice).loopend = (*(*voice).sample)
                    ...
                    .wrapping_add(
                        (32768 as i32
                            * ((*voice).gen[GEN_ENDLOOPADDRCOARSEOFS as i32 as usize].val
                                as f32
                                + (*voice).gen[GEN_ENDLOOPADDRCOARSEOFS as i32 as usize]
                                    .mod_0 as f32
                                + (*voice).gen[GEN_ENDLOOPADDRCOARSEOFS as i32 as usize]
                                    .nrpn as f32) as i32)
                            as u32,
        ...
        33 => {
            ...
            count = ((*voice).output_rate * fluid_tc2sec_delay(x)
                / 64 as i32 as libc::c_float) as u32;
        ...
        34 => {
            ...
            count = (1 as i32 as u32).wrapping_add(
                ((*voice).output_rate * fluid_tc2sec_attack(x) / 64 as i32 as libc::c_float)
                    as u32,
            );
        ...
        38 => {
            ...
            (*voice).volenv_data[FLUID_VOICE_ENVRELEASE as i32 as usize].incr =
                if count != 0 {
                    (-1.0f32) / count as libc::c_float
                } else {
                    0.0f32
        ...
        25 => {
            ...
            (*voice).modenv_data[FLUID_VOICE_ENVDELAY as i32 as usize].count =
                ((*voice).output_rate * fluid_tc2sec_delay(x) / 64 as i32 as libc::c_float)
                    as u32;
        ...
        30 => {
            ...
            (*voice).modenv_data[FLUID_VOICE_ENVRELEASE as i32 as usize].incr =
                if count != 0 {
...
pub unsafe fn fluid_voice_modulate(
    mut voice: *mut fluid_voice_t,
    cc: i32,
    ctrl: i32,
) -> i32 {
    ...
    while i < (*voice).mod_count {
        ...
        if (*mod_0).src1 as i32 == ctrl
            ...
            || ((*mod_0).src2 as i32 == ctrl
                ...
                && cc != 0 as i32
                || (*mod_0).src2 as i32 == ctrl
                    && (*mod_0).flags2 as i32 & FLUID_MOD_CC as i32
                        == 0 as i32
                    && cc == 0 as i32)
        {
...
pub unsafe fn fluid_voice_modulate_all(mut voice: *mut fluid_voice_t) -> i32 {
    let mut mod_0;
    let mut i;
    let mut k;
    let mut gen;
...
pub unsafe fn fluid_voice_noteoff(mut voice: *mut fluid_voice_t) -> i32 {
    let at_tick;
...
pub unsafe fn fluid_voice_kill_excl(mut voice: *mut fluid_voice_t) -> i32 {
    if !((*voice).status as i32 == FLUID_VOICE_ON as i32
        || (*voice).status as i32 == FLUID_VOICE_SUSTAINED as i32)
    {
        return FLUID_OK as i32;
...
pub unsafe fn fluid_voice_off(mut voice: *mut fluid_voice_t) -> i32 {
    (*voice).chan = 0xff as i32 as libc::c_uchar;
...
pub unsafe fn fluid_voice_add_mod(
    mut voice: *mut fluid_voice_t,
    mod_0: &Mod,
    mode: i32,
) {
    ...
    if mode == FLUID_VOICE_ADD as i32 {
        i = 0 as i32;
        while i < (*voice).mod_count {
            if fluid_mod_test_identity(&(*voice).mod_0[i as usize], mod_0)
                != 0
            {
...
pub unsafe fn fluid_voice_get_id(voice: *mut fluid_voice_t) -> u32 {
    return (*voice).id;
}

pub unsafe fn fluid_voice_is_playing(voice: *mut fluid_voice_t) -> i32 {
    return ((*voice).status as i32 == FLUID_VOICE_ON as i32
        || (*voice).status as i32 == FLUID_VOICE_SUSTAINED as i32)
        as i32;
}

pub unsafe fn fluid_voice_get_lower_boundary_for_attenuation(voice: *mut fluid_voice_t) -> f32 {
    let mut i;
    let mut mod_0;
    ...
    if lower_bound < 0 as i32 as libc::c_float {
        lower_bound = 0 as i32 as f32
    }
    return lower_bound;
}

pub unsafe fn fluid_voice_check_sample_sanity(mut voice: *mut fluid_voice_t) {
    ...
    let max_index_loop: i32 =
        (*(*voice).sample).end as i32 - 0 as i32 + 1 as i32;
            ...
            == FLUID_LOOP_DURING_RELEASE as i32
    {
        ...
        if (*voice).loopend < (*voice).loopstart + 2 as i32 {
            (*voice).gen[GEN_SAMPLEMODE as i32 as usize].val =
    ...
    if (*voice).gen[GEN_SAMPLEMODE as i32 as usize].val as i32
        ...
        || (*voice).gen[GEN_SAMPLEMODE as i32 as usize].val as i32
            == FLUID_LOOP_DURING_RELEASE as i32
    {
        let index_in_sample: i32 =
            ((*voice).phase >> 32 as i32) as u32 as i32;
...
pub unsafe fn fluid_voice_set_gain(mut voice: *mut fluid_voice_t, mut gain: f32) -> i32 {
